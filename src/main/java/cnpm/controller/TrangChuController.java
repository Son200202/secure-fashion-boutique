...
//		model.addAttribute("pagedListHolder", pagedListHolder);
	...
	public String timSp(HttpServletRequest request, ModelMap model) {
		...
		for (int i =0 ; i < tensp.length(); i++) {
        	if ((tensp.charAt(i)== '-') || (tensp.charAt(i)== ';') || String.valueOf(tensp.charAt(i)) == "'" 
        			|| (tensp.toLowerCase().contains("union")) || (tensp.toLowerCase().contains("select")) || (tensp.toLowerCase().contains("insert"))
        			|| (tensp.toLowerCase().contains("update")) || (tensp.toLowerCase().contains("delete")) || (tensp.toLowerCase().contains("drop"))
        			|| (tensp.toLowerCase().contains("sp_addlogin")) || (tensp.toLowerCase().contains("sp_password")) || (tensp.toLowerCase().contains("xp_cmdshell"))
        			|| (tensp.toLowerCase().contains("exec")) || (tensp.toLowerCase().contains("sp_send_dbmail"))) {
       		return "shop/SQL_Injection";
        	}
        }
