...
public class KhachHangController {
	...
	public String getthanhToan(ModelMap model, HttpServletRequest request, HttpSession ss) {
		...
		if (tk != null) {
			model.addAttribute("email", tk.getEmail().toString());
			Random generator = new Random();
			int ramdom = generator.nextInt(99999999) + 100000;
			this.maOTP = String.valueOf(ramdom);
			...
			try {
				...
				helper.setSubject("CHECK OTP!");
				helper.setText("Mã OTP của quý khách là: " + this.maOTP);
	...
	public String muaHang(ModelMap model, HttpSession ss, HttpServletRequest request, RedirectAttributes redirectAttributes,
		...
		if (! OTP.equals(this.maOTP)) {
			errors.rejectValue("hinhThucTT.maHTTT", "donhang", "sai mã OTP");
		}
